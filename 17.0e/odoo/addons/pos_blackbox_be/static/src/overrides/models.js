...
patch(PosStore.prototype, {
    ...
    showBlackboxWarning(code, message) {
        if (this.lastBBWarning != code) {
            ...
            this.env.services.notification.add(message || _t("Unknown blackbox warning"), {
                type: "warning",
            });
        }
    },
    async pushToBlackbox(order) {
        try {
            ...
            this.setDataForPushOrderFromBlackbox(order, dataValue);
            return true;
        } catch (err) {
    ...
    async pushDataToBlackbox(data, action) {
        ...
        return new Promise(async (resolve, reject) => {
            fdm.addListener((data) => {
                ...
                return data.status.status === "connected" || data.status === "success"
                    ? resolve(data)
                    : reject(data);
            });
            await fdm.action({
