...
class CzechVATControlReportCustomHandler(models.AbstractModel):
    ...
    def _report_custom_engine_control_statement(self, options, current_groupby, next_groupby, offset=0, limit=None, domain=None, code=None):
        ...
        if code not in {'a5', 'b3'}:  # All codes except aggregated ones require information on individual moves.
            ...
            where_clause += " AND (account_move_line__move_id.amount_total <= 10000 OR partner.vat IS NULL OR partner.vat = '/' OR account_move_line__move_id.l10n_cz_scheme_code IN ('1', '2'))"
        ...
        if code in {'a4', 'b2'}:
            where_clause += " AND account_move_line__move_id.amount_total > 10000 AND partner.vat IS NOT NULL AND partner.vat != '/' AND account_move_line__move_id.l10n_cz_scheme_code NOT IN ('1', '2')"
