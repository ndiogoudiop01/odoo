...
class TestInterCompanyInvoice(TestInterCompanyRulesCommon):
    ...
    def test_inter_company_invoice_skips_incompatible_fiscal_position(self):
        domestic_country = self.company_b.account_fiscal_country_id
        foreign_country = self.env.ref('base.be')
        if foreign_country == domestic_country:
            foreign_country = self.env.ref('base.lu')
        self.company_b.partner_id.country_id = domestic_country
        self.company_a.partner_id.country_id = foreign_country
        fiscal_position = self.env['account.fiscal.position'].with_company(self.company_b).create({
            'name': 'Foreign VAT',
        ...
        self._create_post_invoice(self.product_a.id)
        supplier_invoice = self.env['account.move'].with_user(self.res_users_company_b).search([('move_type', '=', 'in_invoice')], limit=1)
        self.assertFalse(supplier_invoice.fiscal_position_id)
        self.assertEqual(supplier_invoice.tax_country_id, domestic_country)
        self.assertEqual(supplier_invoice.invoice_line_ids.tax_ids.country_id, domestic_country)
        self.assertNotEqual(fiscal_position, supplier_invoice.fiscal_position_id)
