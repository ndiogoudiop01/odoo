...
class TestCFDIInvoice(TestMxEdiCommon):
    ...
    def test_cfdi_rounding_13(self):
        ...
        def run(rounding_method):
            with self.mx_external_setup(self.frozen_today):
                ...
                with self.with_mocked_pac_sign_success():
                    invoice._l10n_mx_edi_cfdi_invoice_try_send()
                self._assert_invoice_cfdi(invoice, f'test_cfdi_rounding_13_{rounding_method}_inv')

                payment = self._create_payment(invoice)
                with self.with_mocked_pac_sign_success():
                    payment.move_id._l10n_mx_edi_cfdi_payment_try_send()
                self._assert_invoice_payment_cfdi(payment.move_id, f'test_cfdi_rounding_13_{rounding_method}_pay')

        today = self.frozen_today
        today_minus_1 = self.frozen_today - relativedelta(days=1)
        self.setup_rates(self.usd, (today_minus_1, 0.049216958195), (today, 0.053418803419))
        self._test_cfdi_rounding(run)

    def test_cfdi_line_discount_amplified_rounding(self):
        """ 'price_subtotal' is rounded at the currency precision; dividing it by the discount factor
        amplifies that rounding and reports a completely wrong 'ValorUnitario'. """
        with self.mx_external_setup(self.frozen_today):
            for price_unit, valor_unitario, descuento in (
                (150.0, '150.00', '149.99'),
                (250.0, '250.00', '249.98'),
            ):
                with self.subTest(price_unit=price_unit):
                    invoice = self._create_invoice(
                    ...
                    # 'discount' is stored as '99.99' but cached as 99.99000000000001: the CFDI,
                    # generated in a later request, doesn't recompute the same 'price_subtotal'.
                    invoice.invoice_line_ids.invalidate_recordset(['discount'])

                    with self.with_mocked_pac_sign_success():
                        invoice._l10n_mx_edi_cfdi_invoice_try_send()
                    document = invoice.l10n_mx_edi_invoice_document_ids\
                        .filtered(lambda x: x.state == 'invoice_sent')
                    concepto = etree.fromstring(document.attachment_id.raw).find('{*}Conceptos/{*}Concepto')
                    self.assertEqual(concepto.get('ValorUnitario'), valor_unitario)
                    self.assertEqual(concepto.get('Importe'), valor_unitario)
                    self.assertEqual(concepto.get('Descuento'), descuento)
    ...
    def test_global_invoice_sequence_access_right(self):
        with self.mx_external_setup(self.frozen_today):
            self.env.company.l10n_mx_edi_global_invoice_sequence_id = self.env.company._create_l10n_mx_edi_global_invoice_sequence()
            invoice = self._create_invoice(
                l10n_mx_edi_cfdi_to_public=True,
                invoice_line_ids=[
                    Command.create({
            ...
            self.simple_accountman.email = 'simple.accountman@example.com'
            with self.with_mocked_pac_sign_success():
                self.env['l10n_mx_edi.global_invoice.create']\
                    .with_context(invoice.l10n_mx_edi_action_create_global_invoice()['context'])\
