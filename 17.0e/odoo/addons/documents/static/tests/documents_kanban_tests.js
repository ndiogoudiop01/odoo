...
QUnit.module("documents", {}, function () {
    QUnit.module(
        ...
        function () {
            ...
            QUnit.test("documents List: preserve selection after non-folder edit", async function (assert) {
                assert.expect(10);
                ...
                await createDocumentsView({
                    ...
                    mockRPC(route, args) {
                        if (args.model === "documents.document" && args.method === "write") {
                            const [ids, vals] = args.args;
                            assert.ok(Array.isArray(ids) && ids.length >= 1, "write targets selected ids");
                            assert.ok(("owner_id" in vals) || ("partner_id" in vals), "write changes a non-folder field");
                            assert.notOk("folder_id" in vals, "write does not change folder_id");
                        }
                    },
                });

                await legacyClick(target.querySelector(".o_data_row:nth-of-type(1) input"));
                await legacyClick(target.querySelector(".o_data_row:nth-of-type(4) input"));
                await nextTick();
                assert.containsN(target, ".o_list_record_selector input:checked", 2, "two rows selected");

                const owner_input = await clickOpenM2ODropdown(target, "owner_id");
                await editInput(owner_input, null, "De Bruyne");
                await clickOpenedDropdownItem(target, "owner_id", "De Bruyne");
                await legacyClick(target.querySelector(".modal-footer .btn-primary"));
                await nextTick();

                const contact_input = await clickOpenM2ODropdown(target, "partner_id");
                await editInput(contact_input, null, "Hazard");
                await clickOpenedDropdownItem(target, "partner_id", "Hazard");
                await legacyClick(target.querySelector(".modal-footer .btn-primary"));
                await nextTick();

                assert.containsN(target, ".o_list_record_selector input:checked", 2, "same number of selected rows remains checked");
                assert.strictEqual(target.querySelector(".o_field_many2one[name=owner_id] input").value, "De Bruyne", "edited Owner value is displayed");
                assert.strictEqual(target.querySelector(".o_field_many2one[name=partner_id] input").value, "Hazard", "edited Contact value is displayed");
            });
            ...
            QUnit.test(
                "documents list: don't unselect record when clicking inside a search modal dialog",
                async function (assert) {
                    assert.expect(3);
                    await createDocumentsView({
                        type: "list",
                        ...
                        serverData: {
                            ...
                            views: {
                                "res.partner,false,list": `<tree><field name="name"/></tree>`,
                            },
                        },
                    });
                    await triggerEvent(target.querySelector(".o_data_row"), null, "keydown", {
                        key: "Enter",
                    });
                    assert.containsOnce(target, ".o_data_row_selected");
                    const input = await clickOpenM2ODropdown(target, "partner_id");
                    await editInput(input, null, "ra");
                    await legacyClick(
                        target.querySelector(".o-autocomplete--dropdown-menu .o_m2o_dropdown_option_search_more")
                    );
                    assert.containsOnce(document.body, ".modal");
                    // Click inside modal list view
                    await legacyClick(document.body.querySelector(".modal th"));
                    assert.containsOnce(target, ".o_data_row_selected");
                }
            );
