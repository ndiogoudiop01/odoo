...
export class PivotAutofillPlugin extends UIPlugin {
    ...
    getPivotNextAutofillValue(formula, isColumn, increment) {
        ...
        if (functionName === "ODOO.PIVOT") {
            ...
            if (evaluatedArgs.length === 1) {
                ...
                this.getters.getPivotDefinition(pivotId).rowGroupBys.includes(nonPositionalArgs[1])
            ) {
                builder = this._autofillPivotRowHeader.bind(this);
            } else {
                builder = this._autofillPivotColHeader.bind(this);
    ...
    _tooltipFormatPivot(pivotId, args, isColumn) {
        ...
        for (let i = 2; i <= domain.length; i += 2) {
            ...
            if (
                ...
                const formattedValue = this._getPivotHeaderFormattedValue(
                    pivotId,
                    domain.slice(0, i)
                );
    ...
    _tooltipFormatPivotHeader(pivotId, args) {
        ...
        for (let i = 2; i <= domain.length; i += 2) {
            const formattedValue = this._getPivotHeaderFormattedValue(pivotId, domain.slice(0, i));
            tooltips.push({ value: formattedValue });
        }
        return tooltips;
    }

    _getPivotHeaderFormattedValue(pivotId, domain) {
        try {
            return this.getters.getPivotHeaderFormattedValue(pivotId, domain);
        } catch {
            return _t("Unknown");
        }
    }
