...
QUnit.module("spreadsheet > pivot_autofill", {}, () => {
    ...
    QUnit.test("Can autofill positional row headers horizontally", async (assert) => {
        const { model } = await createSpreadsheetWithPivot({
            arch: /*xml*/ `
                <pivot>
        ...
        setCellContent(model, "A3", `=ODOO.PIVOT.HEADER(1,"#product_id",1)`);
        assert.strictEqual(
            getPivotAutofillValue(model, "A3", { direction: "right", steps: 1 }),
            `=ODOO.PIVOT(1,"probability","date:month","04/2016")`
        );
        selectCell(model, "A3");
        model.dispatch("AUTOFILL_SELECT", { col: 1, row: 3 });
        const tooltipContent = model.getters.getAutofillTooltip().props.content;
    ...
    QUnit.test("Autofill does not crash when autofilling wrong relational id", async (assert) => {
        const { model } = await createSpreadsheetWithPivot({
            arch: /*xml*/ `
                <pivot>
                    <field name="product_id"  type="col"/>
        ...
        const formula = `=ODOO.PIVOT.HEADER(1,"product_id",9999)`;
        assert.deepEqual(model.getters.getTooltipFormula(formula), [{ value: "Unknown" }]);
    });
});
