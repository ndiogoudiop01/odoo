...
class TestVATBooksPosExportXLSX(TestL10nEsVATBooksPosCommon):
    ...
    def _get_sheet_vals_for_order(self, order):
        ...
        invoice_name = self.env['account.generic.tax.report.handler']._l10n_es_libros_get_pos_order_invoice_number(order)
        return sorted(
            (
    ...
    def test_pos_order_mixed_tax_refund_xlsx_export(self):
        ...
        with self.with_pos_session():
            ...
            order = self.env['pos.order'].browse(results[0]['id'])
        ...
        with self.with_pos_edi_mode('tbai'):
            sheet_vals_list = sorted(
                self.env['account.generic.tax.report.handler']._l10n_es_libros_get_pos_order_sheet_line_vals(refund),
                key=lambda lv: lv['tax_rate'],
            )
    ...
    def test_pos_closing_entry_xlsx_export_single_line_without_order_level_edi(self):
        ...
        self.assertSheetLineVals(line_vals_list[0], {
            'invoice_type': 'F4',
            'invoice_number': 'PoS Shop Test/0001',
            'invoice_final_number': 'PoS Shop Test/0002',
            'base_amount': 300.0,
            'tax_rate': 21.0,
            'taxed_amount': 63.0,
            'total_amount': 363.0,
