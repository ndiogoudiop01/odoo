...
from odoo.addons.l10n_es_reports.models.libros_export import INCOME_FIELDS
...
class GenericTaxReportCustomHandler(models.AbstractModel):
    ...
    def _l10n_es_libros_is_income_line(self, line):
        return (
            ...
            and bool(line.move_id.l10n_es_reports_pos_session_ids)
    ...
    def _l10n_es_libros_create_income_line_vals(self, line, tax):
        ...
        if session := line.move_id.l10n_es_reports_pos_session_ids[:1]:
            if line.is_refund:
                line_vals['invoice_number'] = session.name
            elif active_orders := session.order_ids.filtered(lambda o: not o.refunded_order_ids).sorted('id'):
    ...
    def _l10n_es_libros_get_pos_order_sheet_line_vals(self, pos_order):
        ...
        aggregated = self.env['account.tax']._l10n_es_get_tax_details_for_report(base_lines, company)

        tax_detail_items = [(k, v) for k, v in aggregated['tax_details'].items() if k]
        # Map each main tax key to its paired recargo key (matched by recargo tax amount)
        recargo_keys = [k for k in aggregated['tax_details'] if k['l10n_es_type'] == 'recargo']
        recargo_by_main_key = {}
        ...
        for key, detail in tax_detail_items:
            ...
            tax_amount = detail['tax_amount'] or 0
            ...
            if tax_type in sujeto_tax_types:
                ...
                if recargo_key:
                    recargo_detail = aggregated['tax_details'][recargo_key]
                    recargo_percentage = recargo_key['amount']
                    recargo_amount = recargo_detail['tax_amount'] or 0
    ...
    def _l10n_es_libros_fill_content(self, sheet_income, sheet_expense, report, options):
        ...
        if not self.env.company._l10n_es_get_pos_edi_mode():
            ...
            pos_closing_domain = report._get_options_domain(options, 'strict_range') + [
                ...
                ('move_id.l10n_es_reports_pos_session_ids', '!=', False),
            ]
