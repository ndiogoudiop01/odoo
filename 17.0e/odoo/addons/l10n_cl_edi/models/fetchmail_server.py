...
class FetchmailServer(models.Model):
    ...
    def _process_incoming_customer_claim(self, company_id, att_content, att_name, origin_type):
        ...
        for dte in xml_content.xpath('//ns0:%s' % dte_tag, namespaces=XML_NAMESPACES):
            ...
            # It is possible that multiple partners share the same VAT. For instance, that is the case
            # when a parent partner has a child invoice contact.
            partners = self._get_partner(issuer_vat, company_id, limit=False)
            if not partners:
    ...
    def _get_partner(self, partner_rut, company_id, limit=1):
