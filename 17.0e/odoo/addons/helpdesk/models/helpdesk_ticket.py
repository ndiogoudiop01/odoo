...
class HelpdeskTicket(models.Model):
    ...
    def _find_or_create_partner(self, partner_name, partner_email, company=False):
        ...
        formatted_email = tools.formataddr((parsed_name, parsed_email_normalized))

        return self.with_context(default_company_id=company)._mail_find_partner_from_emails(
            [formatted_email],
            force_create=True,
            extra_domain=[('company_id', 'in', [False, company])] if company else False
        )[0]
