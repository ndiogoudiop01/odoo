 ...
 */
helper.assertLinePackage = (lineOrIndex, expectedPackageName = false) => {
    const line = helper._getLineOrFail(lineOrIndex, "Can't check the line's result package");
    const packageEl = line.querySelector("div[name='package'] > .package");
    if (!packageEl) {
        if (expectedPackageName) {
            helper.fail(`There is no package: ${expectedPackageName} expected`);
        }
    } else {
        const linePackageName = packageEl.innerText;
        helper.assert(linePackageName, expectedPackageName, "Not the expected line's package");
    }
}
 ...
 * @param {string} [expectedPackageName]
 */
helper.assertLineResultPackage = (lineOrIndex, expectedPackageName = false) => {
    const line = helper._getLineOrFail(lineOrIndex, "Can't check the line's result package");
    const resultPackageEl = line.querySelector(".result-package");
    if (!resultPackageEl) {
        if (expectedPackageName) {
            helper.fail(`There is no result package: ${expectedPackageName} expected`);
        }
    } else {
        const linePackageName = resultPackageEl.innerText;
        helper.assert(linePackageName, expectedPackageName, "Not the expected result package");
    }
}
