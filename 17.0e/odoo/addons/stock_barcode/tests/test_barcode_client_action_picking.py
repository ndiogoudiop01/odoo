...
class TestPickingBarcodeClientAction(TestBarcodeClientAction):
    ...
    def test_internal_pack_in_same_package(self):
        ...
        action_id = self.ref('stock_barcode.stock_barcode_action_main_menu')
        self.start_tour(f'/web#action={action_id}', 'test_internal_pack_in_same_package', login='admin')
    ...
    def test_highlight_packs(self):
        ...
        self.clean_access_rights()
        self.env.user.write({'groups_id': [Command.link(self.ref('stock.group_tracking_lot'))]})
        ...
        out_picking = self.env['stock.picking'].create({
            ...
            'package_level_ids': [Command.create({
                'location_id': self.stock_location.id,
                'location_dest_id': self.customer_location.id,
                'package_id': pack1.id,
                'is_done': False,
    ...
    def test_scan_and_exit_unconfirmed_serial_number_picking(self):
        """ Check that when a picking is not assigned, exiting it on barcode
        app does not assign the remaingin quantity (could be problematic if the
        assigned sn quant is not the one that the user wants to scan).
        """
        self.clean_access_rights()
        grp_lot = self.env.ref('stock.group_production_lot')
        self.env.user.write({'groups_id': [Command.link(grp_lot.id)]})
        sn_1, sn_2, sn_3, sn_4 = self.env['stock.lot'].create([
            {'name': f'sn-00{i}', 'product_id': self.productserial1.id, 'company_id': self.env.company.id} for i in range(1, 5)
        ])
        for sn in [sn_1, sn_2, sn_3, sn_4]:
            self.env['stock.quant']._update_available_quantity(self.productserial1, self.stock_location, 1, lot_id=sn)
        ...
        url = self._get_client_action_url(picking.id)
        self.start_tour(url, 'test_scan_and_exit_unconfirmed_serial_number_picking', login='admin')
        self.assertRecordValues(picking.move_ids.move_line_ids, [
            {'product_id': self.productserial1.id, 'quantity': 1, 'lot_id': sn_1.id},
            {'product_id': self.productserial1.id, 'quantity': 1, 'lot_id': sn_2.id},
        ])

    def test_split_line_keeps_origin_package(self):
        '''
        Test that the origin package is kept when splitting a line (eg by putting
        part of the quant in one package and the rest in another).
        '''
        self.clean_access_rights()
        grp_pack = self.env.ref('stock.group_tracking_lot')
        self.env.user.write({'groups_id': [Command.link(grp_pack.id)]})
        origin_pack = self.env['stock.quant.package'].create({'name': 'Origin Package'})

        self.env['stock.quant']._update_available_quantity(self.product1, self.stock_location, 5, package_id=origin_pack)
        ...
        url = self._get_client_action_url(delivery_with_move.id)
        self.start_tour(url, 'test_split_line_keeps_origin_package', login='admin', timeout=180)
        self.assertEqual(delivery_with_move.move_line_ids[0].package_id, origin_pack)
        self.assertEqual(delivery_with_move.move_line_ids[1].package_id, origin_pack)
    ...
    def test_quantity_updates_on_exit_spam(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("stock_barcode.stock_barcode_action_main_menu")
        url = f'/web#action={action["id"]}'
        self.start_tour(url, 'test_quantity_updates_on_exit_spam', login='admin')
        self.assertRecordValues(delivery.move_ids, [{
