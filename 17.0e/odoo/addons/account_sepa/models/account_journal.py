...
class AccountJournal(models.Model):
    ...
    def _get_sepa_country_code(self):
        """ Get fiscal country code from the IBAN, or from the journal. """
        self.ensure_one()
        # First try to retrieve the country_code from the IBAN
        if self.bank_acc_number and re.match(r"[A-Z]{2}[0-9]{2}", self.bank_acc_number):
            country_code = self.bank_acc_number[:2]
        # Then try company's fiscal country, and finally company's country
        else:
            country_code = self.country_code or self.company_id.country_code or ""
        return country_code
    ...
    def _compute_sepa_pain_version(self):
        ...
        for rec in self:
            country_code = rec._get_sepa_country_code()
    ...
    def _get_PmtTpInf(self, sct_generic=False):
        ...
        if self._get_sepa_country_code() not in {"CH", "LI"}:
    ...
    def _get_RmtInf(self, payment):
        ...
        def get_strd_tree(ref, cd=None, prtry=None, issr=None):
            ...
            if cd:
                etree.SubElement(CdOrPrtry, 'Cd').text = cd
            if prtry:
                etree.SubElement(CdOrPrtry, 'Prtry').text = prtry
            if issr:
                etree.SubElement(Tp, 'Issr').text = issr
            etree.SubElement(CdtrRefInf, 'Ref').text = ref
            return Strd
        ...
        elif reference_type in ('dk', 'fi', 'no', 'se'):
            RmtInf.append(get_strd_tree(ref, cd='SCOR'))
        else:
            Ustrd = etree.SubElement(RmtInf, "Ustrd")
            Ustrd.text = sanitize_communication(payment['ref'].replace('&', '+'))
