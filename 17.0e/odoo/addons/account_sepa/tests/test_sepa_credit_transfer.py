...
class TestSEPACreditTransfer(AccountTestInvoicingCommon):
    ...
    def test_sepa_ampersand_in_partner_name(self):
        ...
        payment = self.createPayment(self.partner_a, 500, ref="Invoice & Order")
        payment.action_post()
    ...
    def test_ch_qr_iban_journal_reference_with_special_chars_is_sanitized(self):
        ...
        namespaces = {'ns': 'http://www.six-interbank-clearing.com/de/pain.001.001.03.ch.02.xsd'}
        ct_doc = etree.fromstring(base64.b64decode(batch.export_file))
    ...
    def test_sepa_batch_payment_limit_euro(self):
        payment_1 = self.createPayment(self.partner_a, 99999999.99)
        payment_2 = self.createPayment(self.partner_a, 100000000.00)
        payment_1.action_post()
        payment_2.action_post()

        batch_1 = self.env['account.batch.payment'].create({
            'journal_id': self.bank_journal.id,
            'payment_ids': [payment_1.id],
            'payment_method_id': self.sepa_ct_method.id,
            'batch_type': 'outbound',
        })
        batch_2 = self.env['account.batch.payment'].create({
            'journal_id': self.bank_journal.id,
            'payment_ids': [payment_2.id],
            'payment_method_id': self.sepa_ct_method.id,
            'batch_type': 'outbound',
        })
        ...
        self.assertEqual(batch_1.state, "sent")
        self.assertEqual(batch_2.state, "draft")

    def test_sepa_batch_payment_limit_non_euro(self):
        self.company_data['default_journal_bank'].currency_id = self.ref('base.USD')
        payment_1 = self.createPayment(self.partner_a, 999999999999.99)
        payment_2 = self.createPayment(self.partner_a, 1000000000000.00)
        payment_1.action_post()
        payment_2.action_post()
