    ...
    def _report_custom_engine_kmd_inf(self, options, current_groupby, next_groupby, kmd_inf_part):
        ...
        if current_groupby != 'id':
            ...
            if kmd_inf_part == 'a':
                ...
                # Filter to only include moves that have VAT
                extra_where = f"""
                    AND EXISTS (
                        SELECT 1
                        ...
                        AND tag_rel.account_account_tag_id = ANY(ARRAY{tag_ids})
            ...
            partners_to_exclude_query = f"""
                ...
                AND (res_country.code = 'EE' OR res_partner.vat ILIKE %s)
                {extra_where}
                GROUP BY account_move_line.partner_id
                HAVING
                    COALESCE(
                        ABS(SUM(currency_table.rate * account_move_line.balance)
            ...
            """
            self.env.cr.execute(partners_to_exclude_query, [*where_params, 'EE%'])
            partners_to_exclude = [row[0] for row in self.env.cr.fetchall()]

        tables, where_clause, where_params = report._query_get(options, 'strict_range', [('partner_id', 'not in', partners_to_exclude)])
        where_clause = SQL(where_clause, *where_params)
