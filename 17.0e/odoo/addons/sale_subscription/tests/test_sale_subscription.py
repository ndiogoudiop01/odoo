...
class TestSubscription(TestSubscriptionCommon):
    ...
    def test_subscription_constraint(self):
        ...
        # Test adding recurring product in draft So through catalog
    ...
    def test_upsell_qty_invoiced_next_invoice_date(self):
        """
        Check that qty_invoiced on the upsell order line should reflect the
        actual invoiced quantity instead of its default value 0.0.
        """
        self.subscription.action_confirm()
        self.subscription.next_invoice_date += relativedelta(months=2)
        self.subscription._cron_recurring_create_invoice()
        action = self.subscription.prepare_upsell_order()
        upsell_so = self.env['sale.order'].browse(action['res_id'])
        upsell_line = upsell_so.order_line.filtered(lambda l: not l.display_type)
        upsell_line[0].product_uom_qty = 2
        upsell_so.action_confirm()
        invoice = upsell_so._create_invoices()
        invoice.action_post()
        self.assertEqual(upsell_line[0].qty_invoiced, 2)
