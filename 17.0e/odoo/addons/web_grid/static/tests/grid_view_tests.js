...
QUnit.module("Views", (hooks) => {
    ...
    QUnit.test("Selection field label is displayed instead of its key when clicking on cell magnifier", async function (assert) {
        serverData.views["analytic.line,false,grid"] = `<grid>
            <field name="selection_field" type="row"/>
            <field name="task_id" type="col"/>
            <field name="unit_amount" type="measure" widget="float_time"/>
        ...
        await hoverGridCell(target.querySelectorAll(".o_grid_row .o_grid_cell_readonly")[6]);
        await click(target, ".o_grid_cell button.o_grid_search_btn");

        assert.deepEqual(getNodesTextContent(target.querySelectorAll(".o_control_panel_breadcrumbs .o_last_breadcrumb_item")), ["GHI (BS task)"]);
    });
});
