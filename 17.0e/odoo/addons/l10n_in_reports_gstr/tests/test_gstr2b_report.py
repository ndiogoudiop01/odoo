...
from .gstr_test_json import gstr2b_test_json, gstr2b_late_reconciliation_data, gstr2b_reconciliation_different_partner, gstr2b_draft_bill_test_json, gstr2b_foreign_currency_test_json
import logging
import json
...
class TestReports(TestAccountReportsCommon):
    ...
    def test_bill_status_and_exception_reset_on_draft(self):
        self.report.gstr2b_json_from_portal_ids = self.env['ir.attachment'].create({
            'name': 'gstr2b.json',
            'mimetype': 'application/json',
            'raw': json.dumps(gstr2b_draft_bill_test_json),
        })
        self.report.gstr2b_match_data()
    ...
    def test_gstr2b_foreign_currency_bill(self):
        ...
        foreign_bill = self.l10n_in_reports_gstr2b_inv_init('FC-INV/001', self.partner_b, 100.0)
        ...
        self.report.gstr2b_json_from_portal_ids = self.env['ir.attachment'].create({
            ...
            'mimetype': 'application/json',
            'raw': json.dumps(gstr2b_foreign_currency_test_json),
        })
        self.report.gstr2b_match_data()
