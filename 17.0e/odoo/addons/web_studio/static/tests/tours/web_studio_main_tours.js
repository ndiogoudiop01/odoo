...
registry.category("web_tour.tours").add("web_studio_rename", {
    ...
    steps: () => [
        ...
        {
            // rename the label
            trigger: '.o_web_studio_sidebar_text input[name="string"]',
            run: "text السَّلَامُ عَلَيْكُمْ", // As-salamu alaykum
        },
        stepNextTick(),
        {
            ...
            trigger: '.o_web_studio_sidebar_text input[name="string"]',
            run(helper) {
                assertEqual(this.$anchor[0].value, "السَّلَامُ عَلَيْكُمْ");
            },
            // the rename operation (/web_studio/rename_field + /web_studio/edit_view)
            // takes a while and sometimes reaches the default 10s timeout
            timeout: 20000,
        },
        {
            ...
            trigger: '.o_web_studio_sidebar_text input[name="technical_name"]',
            run(helper) {
                const { value } = this.$anchor[0];
                if (!value.startsWith("char_field_")) {
                    throw new Error(`Assert failed: expected string to starts with "char_field+", got: "${value}"`);
                }
            },
        },
        {
...
registry.category("web_tour.tours").add("web_studio_test_cohort_measure_values", {
    ...
    steps: () => [
        ...
        },
        {
            // open cohort view
            trigger:
                ".o_web_studio_view_category .o_web_studio_view_type.o_web_studio_inactive[data-type='cohort'] .o_web_studio_thumbnail",
            run: "click",
        },
        {
