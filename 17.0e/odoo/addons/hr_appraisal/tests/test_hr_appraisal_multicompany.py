...
class TestHrAppraisal(TransactionCase):
    ...
    def test_appraisal_visibility_across_companies(self):
        """
        check that an employee's appraisal marked as 'invisible to the manager'
        remains invisible to the manager when switching companies
        """
        manager_user = new_test_user(
            self.env,
            login='manager',
            groups='hr_appraisal.group_hr_appraisal_user',
            company_ids=[(6, 0, (self.main_company | self.other_company).ids)],
            company_id=self.main_company.id
        )

        manager_employee = self.HrEmployee.create({
            'name': 'Manager employee',
            'company_id': self.main_company.id,
            'user_id': manager_user.id,
        })

        self.hr_employee.parent_id = manager_employee

        appraisal = self.HrAppraisal.create({
            'employee_id': self.hr_employee.id,
            'manager_ids': manager_employee.ids,
            'state': 'pending',
            'employee_feedback_published': False,
        })

        appraisal_with_main_company = appraisal.with_user(manager_user).with_context(allowed_company_ids=[self.main_company.id, self.other_company.id])
        self.assertFalse(appraisal_with_main_company.can_see_employee_publish)

        appraisal.invalidate_recordset(['can_see_employee_publish'])

        appraisal_with_other_company = appraisal.with_user(manager_user).with_context(allowed_company_ids=[self.other_company.id, self.main_company.id])
        self.assertFalse(appraisal_with_other_company.can_see_employee_publish)
