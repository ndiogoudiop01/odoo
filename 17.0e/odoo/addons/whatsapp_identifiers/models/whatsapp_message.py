...
class WhatsappMessage(models.Model):
    ...
    def _send_with_identifier(self, wa_api, **send_kwargs):
        ...
        if self.mobile_number_formatted:
            send_response = super()._send_with_identifier(wa_api, **send_kwargs)
        elif bsuid := self.recipient_wa_bsuid:
            send_response = wa_api._send_whatsapp_to_identifier(bsuid=bsuid, number=None, **send_kwargs)
        if wa_id := send_response.get('wa_id'):
            self.recipient_wa_id = wa_id
