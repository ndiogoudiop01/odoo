...
class TestWhatsAppBsuid(WhatsAppCommon, MockIncomingWhatsApp):
    ...
    def test_bsuid_is_scoped_per_account(self):
        ...
        partner = self.env['res.partner'].search([('mobile', '=', sender_phone)])
    ...
    def test_channel_found_by_wa_id_when_number_format_mismatches(self):
        ...
        self.assertEqual(existing_channel.whatsapp_partner_id.mobile, '+32468000040')
        self.assertEqual(existing_channel.whatsapp_partner_id.phone, False)
        ...
        self.assertEqual(partner.mobile, '+32468000040')  # not updated because we don't know if user updated it on purpose
        self.assertEqual(partner.phone, False)
    ...
    def test_receive_creates_partner_and_channel(self):
        ...
        cases = [
            (
                ...
                {'bsuid': 'BE.11111111111111111', 'mobile': False, 'country_id': belgium},
            ),
            (
                'bsuid-only, with sender name',
                {'sender_bsuid': 'BE.22222222222222222', 'sender_name': 'Hidden User'},
                {'bsuid': 'BE.22222222222222222', 'mobile': False, 'country_id': belgium, 'name': 'Hidden User'},
            ),
            (
                'phone + bsuid',
                {'sender_phone_number': '+32468000011', 'sender_bsuid': 'BE.33333333333333333',
                 'sender_name': 'Both Fields User'},
                {'bsuid': 'BE.33333333333333333', 'mobile': '+32468000011'},
            ),
        ]
    ...
    def test_receive_reuses_existing_channel(self):
        ...
        cases = [
            ('phone-only', {'sender_phone_number': '+32468000030'}, [('mobile', '=', '+32468000030')]),
    ...
    def test_same_bsuid_on_two_partners_does_not_crash(self):
        ...
        user_created_partner = ResPartner.create({'name': 'Known Number', 'mobile': f'+{sender_number}'})
        composer = self._instanciate_wa_composer_from_records(self.simple_whatsapp_template, user_created_partner)
        with self.mockWhatsappGateway():
