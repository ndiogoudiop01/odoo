...
class SocialStreamPostFacebook(models.Model):
    ...
    def _facebook_comment_fetch(self, next_records_token=False, count=20):
        ...
        for comment in result_json.get('data'):
            ...
            if comment.get('attachment', {}).get('type') == 'animated_image_share':
            ...
            for inner_comment in inner_comments:
                ...
                if inner_comment.get('attachment', {}).get('type') == 'animated_image_share':
                    # GIF from the Facebook "GIF button"
                    inner_comment['attachment']['type'] = 'animated_image_video'
