...
class AccountMove(models.Model):
    ...
    def _generate_deferred_entries(self):
        ...
        for line in self.line_ids.filtered(lambda l: l.deferred_start_date and l.deferred_end_date):
            ...
            # When using the 'full_month' computation method, every consumed month counts as a full month.
            # We therefore need to subtract one month from the end date for the following check on dates.
            if line.company_id.deferred_amount_computation_method == 'full_months':
...
class AccountMoveLine(models.Model):
    ...
    def _build_predictive_query(self, additional_domain=None):
        ...
        move_query.order = 'account_move.invoice_date DESC, account_move.id DESC'
