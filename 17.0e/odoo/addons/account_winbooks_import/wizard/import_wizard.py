...
class WinbooksImportWizard(models.TransientModel):
    ...
    def _import_move(self, dbf_records, pdffiles, account_data, account_central, journal_data, partner_data, vatcode_data, param_data):
        ...
        move_dict = {}
        for move_data_split in self.env.cr.split_for_in_conditions(move_data_list):  # Moves created in a loop to avoid creating an excessively large recordset
            move_ids = self.env['account.move'].with_context(skip_invoice_sync=True).create(move_data_split)
            move_ids.invalidate_model()  # Invalidate cache to free up memory
            advancement_counter += len(move_ids)
            _logger.info("Advancement: %s", advancement_counter)
            move_ids._post()
            ...
            move_dict.update({f"{m.date.year}_{m.ref}": m for m in move_ids})
        return move_dict
