...
class GeneralLedgerCustomHandler(models.AbstractModel):
    ...
    def _saft_fill_report_partner_ledger_values(self, options, values):
        ...
        tables, where_clause, where_params = report._query_get(options, 'from_beginning', domain=[
            ('account_id.account_type', 'in', ('asset_receivable', 'liability_payable')),
            ('partner_id', '!=', False),
        ])
        self.env.cr.execute(f"""
            SELECT partner_id,
                BOOL_OR(a.account_type = 'asset_receivable') AS any_receivable,
                BOOL_OR(a.account_type = 'liability_payable') AS any_liability,
                COALESCE(SUM(balance) FILTER (WHERE date < %s AND a.account_type = 'asset_receivable'), 0) AS opening_receivable,
                COALESCE(SUM(balance) FILTER (WHERE a.account_type = 'asset_receivable'), 0) AS closing_receivable,
                COALESCE(SUM(balance) FILTER (WHERE date < %s AND a.account_type = 'liability_payable'), 0) AS opening_payable,
                COALESCE(SUM(balance) FILTER (WHERE a.account_type = 'liability_payable'), 0) AS closing_payable
              FROM {tables}
              JOIN account_account a ON account_id = a.id
             WHERE {where_clause}
          GROUP BY partner_id
            HAVING MIN(date) FILTER (WHERE date >= %s AND date <= %s) IS NOT NULL
        """, [
        ...
        balance_result = self.env.cr.fetchall()
