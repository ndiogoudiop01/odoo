...
class LuxembourgishFinancialReportCustomHandler(models.AbstractModel):
    ...
    def get_xml_2_0_report_values(self, options, references=False):
        ...
        def _get_references(report):
            ...
            """
            notes = self.env['account.report.footnote'].search([
                ('report_id', '=', report.id),
            ])
            if not notes:
                return {}, {}
            ...
            account_ids = set()
            for note in notes:
                report_line_ids, account_id = [], None
                parsed = report._parse_line_id(note.line_id)
                for _markup, model, value in parsed:
                    if model == 'account.report.line':
                        report_line_ids.append(value)
                    elif model == 'account.account':
                        account_id = value
                if report_line_ids and account_id:
                    line_ids.update(report_line_ids)
                    account_ids.add(account_id)
                    mapping.append((note, report_line_ids, account_id))

            if not mapping:
                return {}, {}

            lines = {l.id: l for l in self.env['account.report.line'].browse(line_ids)}
            accounts = {a.id: a for a in self.env['account.account'].browse(account_ids)}

            references, names = {}, {}
            digit_pattern = re.compile(r'\d+')
            for note, report_line_ids, account_id in mapping:
                for report_line_id in report_line_ids:
                    line = lines.get(report_line_id)
                    if not line:
                        continue
                    if match := digit_pattern.search(str(line.code)):
                        ...
                        if account := accounts[account_id]:
