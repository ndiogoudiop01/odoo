...
class LuxembourgElectronicReportTest(TestAccountReportsCommon):
    ...
    @freeze_time('2019-12-31')
    def test_xml_export_with_footnotes(self):
        """Test that footnotes on the P&L report are exported as references when the checkbox is checked."""
        ...
        pl_line = self.env['account.report.line'].search([
            ('report_id', '=', pl_report.id),
            ('code', 'like', '%701'),
        ], limit=1)
        account = self.company_data['default_account_revenue']
        footnote_line_id = pl_report._build_line_id([
            (None, 'account.report.line', pl_line.id),
            (None, 'account.account', account.id),
        ])
        self.env['account.report.footnote'].create({
            'report_id': pl_report.id,
            'line_id': footnote_line_id,
            'text': 'NoteRef',
        })
    ...
    def test_generate_balance_sheet_xml_current_year_earnings(self):
        ...
        account_142 = self.env['account.account'].search([('code', '=', '142000'), ('company_id', '=', company_id)], limit=1)
        account_999 = self.env['account.account'].search([('code', '=', '999999'), ('company_id', '=', company_id)], limit=1)
        if not account_999:
