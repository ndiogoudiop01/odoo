...
class TestBarcodeQualityControlClientAction(TestBarcodeClientAction, TestQualityCommon):
    ...
    def test_operation_quality_check_multi_step_internal_barcode(self):
        """
        Test quality check on multi-step internal transfert from barcode.
        """

        internal_shelf_picking_type = self.env['stock.picking.type'].create({
            'name': 'Internal Stock → SHELF1',
            'code': 'internal',
            'barcode': 'SHELF1',
            'warehouse_id': self.stock_location.warehouse_id.id,
            'default_location_src_id': self.stock_location.id,
            'default_location_dest_id': self.shelf1.id,
            'sequence_code': 'INTSHELF1',
        })
        self.env['quality.point'].create([
            {
                'title': "check product 1",
                'measure_on': "operation",
                'picking_type_ids': [Command.link(internal_shelf_picking_type.id)],
            },
        ])
        route = self.env['stock.route'].create({
            ...
            'rule_ids': [Command.create({
                'name': 'S1 -> S2',
                'action': 'push',
                'location_src_id': self.shelf1.id,
                'location_dest_id': self.shelf2.id,
                'picking_type_id': internal_shelf_picking_type.id,
                'auto': 'manual',
            })]
        })
        self.product1.route_ids = [Command.link(route.id)]
        action_id = self.env.ref('stock_barcode.stock_barcode_action_main_menu')
        url = "/web#action=" + str(action_id.id)

        self.start_tour(url, 'test_operation_quality_check_multi_step_internal_barcode', login='admin')
        quality_check = self.env['quality.check'].search([
            ('point_id.measure_on', '=', 'operation'),
            ('picking_id.picking_type_id', '=', internal_shelf_picking_type.id),
        ], limit=1)
        self.assertEqual(quality_check.quality_state, 'pass')
        self.assertEqual(quality_check.picking_id.state, 'done')
