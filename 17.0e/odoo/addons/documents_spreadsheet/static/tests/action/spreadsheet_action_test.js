...
QUnit.module(
    ...
    function () {
        ...
        QUnit.test(
            "Cancel callback of askConfirmation is called when using the cancel button",
            async function (assert) {
                ...
                env.askConfirmation(
                    ...
                    () => {},
                    () => {
                        assert.step("cancel callback");
                    }
                );
                await nextTick();
                assert.equal(document.querySelector(".o_dialog .btn-primary").innerText, "Yes");
                assert.equal(document.querySelector(".o_dialog .btn-secondary").innerText, "No");
                await click(document, ".o_dialog .btn-secondary");
                assert.verifySteps(["cancel callback"]);
            }
        );
        ...
        QUnit.test("Grid has still the focus after a dialog", async function (assert) {
            ...
            assert.strictEqual(
                ...
                document.querySelector(".o-grid div.o-composer")
            );
        });
