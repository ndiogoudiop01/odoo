...
from odoo.tools import pycompat, float_repr, float_compare
    ...
    def _l10n_de_datev_build_line(
        ...
        *,
        ...
        if currency != company_currency:
            ...
            rate = line_amount / line_amount_currency if not currency.is_zero(line_amount_currency) else 1.0
        ...
        for m in moves:
            ...
            if (m.move_type == 'in_invoice' and
                ...
                actual_values_by_group = {
                    self.env['account.tax.group'].browse(group['tax_group_id']): group['tax_group_amount']
                    for groups in m.tax_totals['groups_by_subtotal'].values()
                    for group in groups
                }
            ...
            for aml in m.line_ids:
                ...
                if aml.currency_id != aml.company_id.currency_id:
                    rate = abs(aml.balance / aml.amount_currency) if aml.amount_currency else 1.0
                    line_amount = aml.company_id.currency_id.round(line_amount_currency * rate)
                else:
