...
class TestJournalAuditReport(TestAccountReportsCommon):
    ...
    def test_journal_report_bank_load_more(self):
        """ Test that the load more button works correctly with bank journal including unreconciled payments. """
        report = self.env.ref('account_reports.journal_report')
        bank_journal = self.company_data['default_journal_bank']
        options = self._generate_options(
            report,
            fields.Date.from_string('2026-05-01'),
            fields.Date.from_string('2026-05-31'),
        )
        report.load_more_limit = 3

        move_specs = [
            ('2026-05-16', self.company_data['default_account_receivable'], 200.0),
            ('2026-05-16', self.liquidity_account, 200.0),
            ('2026-05-16', self.liquidity_account, 200.0),
        ]
        ...
        moves = self.env['account.move'].create([
            ...
            for date, debit_account, amount in move_specs
        ])
        ...
        self.assertLinesValues(
            [l for l in lines if l.get('columns')],
            #   Name                 Account                         Debit              Credit                  Balance
            [   0,                   2,                              4,                 5,                      6],
            [
                ('Name',             'Account',                      'Debit',           'Credit',               'Balance'),
                ('',                 '',                             '',                'Starting Balance:',    '$\xa0300.00'),
                ('BNK1/2026/00001',  '121000 Account Receivable',    '$\xa0200.00',     '$\xa00.00',            ''),
                ('',                 '400000 Product Sales',         '$\xa00.00',       '$\xa0200.00',          ''),
                ('Load more...',     '',                             '',                '',                     ''),
            ],
            options,
        )

        load_more_res = report._expand_unfoldable_line(
            '_report_expand_unfoldable_line_journal_report',
            lines[0]['id'],
            None,
            options,
            lines[5]['progress'],
        ...
        self.assertLinesValues(
            [l for l in load_more_res if l.get('columns')],
            #   Name                 Account                        Debit               Credit              Balance
            [   0,                   2,                             4,                  5,                  6],
            [
                ('BNK1/2026/00002',  '400000 Product Sales',        '$\xa00.00',        '$\xa0200.00',      '$\xa0200.00'),
                ('Load more...',     '',                            '',                 '',                 ''),
            ],
            options,
        )
