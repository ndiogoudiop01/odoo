...
class AccountReport(models.Model):
    ...
    def _get_date_bounds_info(self, options, date_scope):
        ...
        if date_scope == 'normal':
            ...
            current_period_start, _current_period_end = self.env.company._get_tax_closing_period_boundaries(fields.Date.from_string(options['date']['date_from']))
            eve_of_period_start = current_period_start - relativedelta(days=1)
            date_from, date_to = self.env.company._get_tax_closing_period_boundaries(eve_of_period_start)
        ...
        def create_hierarchy_line(account_group, column_totals, level, parent_id):
            ...
            for col_total, column in zip(column_totals, options['columns']):
                ...
                if isinstance(col_total, tuple):
                    col_value, currency_id = col_total
                    if currency_id:
    ...
    def _inject_report_into_xlsx_sheet(self, options, workbook, sheet):
        lines = self._filter_out_folded_children(self.with_context(no_format=True)._get_lines(options))
        y_offset = self._inject_columns_and_headers_in_sheet(workbook, sheet, options, lines)
        self._inject_lines_into_xlsx_sheet(workbook, sheet, lines, options, y_offset)

    def _inject_columns_and_headers_in_sheet(self, workbook, sheet, options, lines):
        ...
        if has_account_lines:
            sheet.set_column(0, 0, 11)
            sheet.set_column(1, 1, 50)
        else:
            sheet.set_column(0, 0, 50)

        original_x_offset = 1 if has_account_lines else 0
    ...
    def _inject_lines_into_xlsx_sheet(self, workbook, sheet, lines, options, y_offset, x_offset=None):
        ...
        if x_offset is None:
            x_offset = 1 if account_lines_split_names else 0
        for y, line in enumerate(lines):
            ...
            if lines[y]['id'] in account_lines_split_names:
                ...
                write_method(y + y_offset, x_offset, cell_value, cell_format)

                if 'parent_id' in line and line['parent_id'] in account_lines_split_names:
                    sheet.write(y + y_offset, 1 + x_offset, account_lines_split_names[line['parent_id']][0], account_code_cell_format)
                elif account_lines_split_names:
                    sheet.write(y + y_offset, 1 + x_offset, "", account_code_cell_format)

            # Write all the remaining cells.
            columns = line['columns']
            if options['show_growth_comparison'] and 'growth_comparison_data' in line:
                columns += [line['growth_comparison_data']]
            for x, column in enumerate(columns, start=1 + x_offset):
...
class AccountReportExternalValue(models.Model):
    ...
    def _build_vals_to_check_for_lock_date(self, records):
        ...
        for external_value in records:
            ...
            yield (
                not self.env.context.get('ignore_tax_lock_date') and generic_tax_report in (report + report.root_report_id + report.section_main_report_ids.root_report_id),  # is_tax
    ...
    def _check_lock_date_violation(self, vals_to_check):
        ...
        """
        for is_tax, date, company in vals_to_check:
            violated_lock_dates = company._get_violated_lock_dates(date, is_tax)
            if violated_lock_dates:
                lock_dates = "".join([f"\n- {lock_date_name} ({format_date(self.env, lock_date)})" for lock_date, lock_date_name in violated_lock_dates])
                raise ValidationError(_("Locked by the following lock dates: %s", lock_dates))
