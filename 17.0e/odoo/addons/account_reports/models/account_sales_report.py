...
class ECSalesReportCustomHandler(models.AbstractModel):
    ...
    def _get_query_sums(self, report, options):
        ...
        for column_group_key, column_group_options in report._split_options_per_column_group(options).items():
            ...
            if allowed_ids:
                where_clause += f" AND {tax_elem_table}.id IN %s"
                params.append(tuple(allowed_ids))

                if service_ids:
                    where_clause += f"AND ({tax_elem_table}.id NOT IN %s OR LEFT(res_partner.vat, 2) IS DISTINCT FROM 'XI')"
                    params.append(tuple(service_ids))
